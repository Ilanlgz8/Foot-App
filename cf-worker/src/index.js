...
async function runOnePass(env) {
  ...
  for (const { slug, evt } of allEvents) {
   ...
   try {
    ...
    // ⏳ Mi-temps (11/09, retour au sablier après essai 🟡 point jaune le même jour —
    // demande explicite utilisateur, gardé simple)
...
// ⚠️ ÉTUDIÉ, DÉPLOYÉ PUIS REVERTÉ (28/08, optimisation latence notifs) :
// l'idée était de faire tourner runOnePass() 2 fois dans la même invocation
// (même technique que recheckFinalMatch : setTimeout dans un ctx.waitUntil)
// pour réduire la latence moyenne de détection (~30s → ~15s), sans
// invocation Worker ni appel Vercel supplémentaire.
// Reverté après un vrai incident (retour utilisateur : silence total sur les
// notifs pendant un pic de 5 matchs simultanés, puis rattrapage isolé et très
// en retard). Root cause identifiée : le plafond Workers gratuit de 50
// sous-requêtes externes par invocation (ESPN + Upstash Redis confondus,
// voir SUBREQUEST_SAFE_LIVE_THRESHOLD plus haut — déjà calé par une session
// précédente pour 1 SEULE passe) était compté correctement pour 1 passe,
// mais le passage à 2 passes fait consommer ce même budget 2x DANS LA MÊME
// invocation. Un seul match chargé (plusieurs buts + FT confirmé) coûte à
// lui seul ~15 sous-requêtes (dédup+envoi Vercel par but/carton, écritures
// finalDone/résumé/warm FD.org sur le FT) — avec 5 matchs simultanés, une
// passe peut déjà approcher les 30-35, la 2e passe 25s après pousse au-delà
// des 50 → Cloudflare interrompt l'exécution EN PLEIN TRAITEMENT, expliquant
// exactement les 2 symptômes remontés (silence pendant le pic, rattrapage
// tardif une fois qu'une passe finit par passer sous le plafond).
// Gain réel (~15s de latence en moins) pas assez pour justifier de re-tenter
