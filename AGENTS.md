...
## Stack
- **Frontend** : React 19, Vite, React Router, React Query, vite-plugin-pwa (Workbox)
...
- `NotificationBell.jsx` — dans la navbar, navigue vers `/favoris` (grand affichage, remplace l'ancien popover)

### Favoris
- `useFavoriteClubs.js` — équipes favorites pour la MISE EN AVANT dans l'UI (localStorage `fav_clubs`, max 10)
- `useFavoriteTeams.js` — nom historique trompeur : c'est en réalité un filtre par COMPÉTITION pour les notifs (localStorage `fav_comps`), pas un favori par équipe
- `pages/FavoritesPage.jsx` (route `/favoris`) — notifs + championnats suivis + équipes favorites, ouverte depuis la cloche
- Couleur par équipe : `getTeamColor(name)` (`data/teamPhotos.js`) utilisée pour teinter étoile/ligne classement, badge, bandeau "Mon équipe" et chips — jamais une couleur générique unique pour toutes les équipes suivies
...
- `StandingsTable.jsx` — composant partagé classement (utilisé dans Classement.jsx, MatchModal et FavoritesPage)
...
src/
  components/
    ...
    FavStarBadge.jsx   — badge étoile équipe favorite (Programme/Résultats)
    Classement.jsx
    navbar.jsx
  pages/
    LiveMatchPage.jsx  — page dédiée /live/:matchId
    LiveMatchPage.css
    FavoritesPage.jsx  — page dédiée /favoris
  hooks/
    ...
    useTeamForm.js
    useFavoriteClubs.js
    useFavoriteTeams.js — filtre notifs par compétition (nom trompeur, voir Favoris)
    liveTracker.js
    useOnline.js
  context/
    LiveProvider.jsx
  ...
  data/
    ...
    teamPhotos.js       — couleurs curées par équipe (getTeamColor, getMatchGradient, getMatchThemeVars)
    teamShortNames.js   — TEAM_SHORT, source unique (utilisé par Live/LiveMatchPage/MatchDuJourCard)
    competitions.js
  App.jsx
  live.css
  matchModal.css
...
- Ce fichier doit rester synchronisé avec `CLAUDE.md` (même contenu, deux conventions de nom
  différentes selon l'outil IA utilisé) — mettre à jour les deux en même temps
