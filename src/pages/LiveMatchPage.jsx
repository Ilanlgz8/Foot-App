...
function MatchHeader({ match, espn, onBack, hForm, aForm, homeCrest, awayCrest }) {
  ...
  return (
    <div className="lmp__heroOuter">
      ...
      <div
        className={`mp__hero lmp__hero${isTermine ? '' : ' sf-liveBorder'}${comp?.tintLight ? ' lmp__hero--lightTint' : ''}${comp?.tint ? ' lmp__hero--tinted' : ''}${comp?.tintSoft ? ' lmp__hero--softTint' : ''}${comp?.tintSilverText ? ' lmp__hero--silverText' : ''}${comp?.tintTheme ? ` lmp__hero--theme-${comp.tintTheme}` : ''}`}
        style={{
